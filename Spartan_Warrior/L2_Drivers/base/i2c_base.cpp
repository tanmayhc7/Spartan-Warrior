...
bool I2C_Base::slave_init(uint8_t* buffer, uint32_t bufferSize)
{
    // Power on I2C
    switch(mIRQ) {
        case I2C0_IRQn: lpc_pconp(pconp_i2c0, true);  break;
    ...
    // Enable I2C and the interrupt for it
    mpI2CRegs->I2CONSET = 0x44; //Init in slave mode

    this->sTransaction.pSlaveData = buffer;
    this->sTransaction.bufferSize = bufferSize;
    NVIC_EnableIRQ(mIRQ);
    return true;
}
...
I2C_Base::mStateMachineStatus_t I2C_Base::i2cStateMachine()
{
    enum {
        ...
        slaveWriteSelectedAckReturned = 0x60,
        slaveDataRecdAckReturned = 0x80,
        slaveStopRecd = 0xA0,

        //slave transmitter states
        slaveReadSelectedAckReturned = 0xA8,
        slaveDataTxedAckRecd = 0xB8,
        slaveDataTxedNackRecd = 0xC0,

        //remaining slave states *Rainy Day Scenarios*
        arbitLostAckdAsWSlave = 0x68,
        genCallRcdAcked = 0x70,
        arbitLostGenCallAcked = 0x78,
        dataRcdNackRetnd = 0x88,
        genCallDataRcdAckRetnd =0x90,
        genCallDataRcdNackRetnd = 0x98,
        arbitLostAckdAsRSlave = 0xB0,
        lastDataTxedAckRecd = 0xC8,
    ...
    #define setStop()           clearSTARTFlag();                           \
                                ...
                                if(I2C_READ_MODE(mTransaction.slaveAddr))   \
                                    ...
                                    state = writeComplete;
    //Slave Related Definitions

    //const uint32_t SIZEOF_SlaveData =6;
    //static uint8_t SlaveData[SIZEOF_SlaveData]= {0xAA,0xBB,0xCC,0xDD,0xEE,0xFF};
    switch (mpI2CRegs->I2STAT)
    {
        ...
        case repeatStart:
            ...
            clearSIFlag();
            break;
        case slaveWriteSelectedAckReturned: //60
            sTransaction.next_is_register = true;
            sTransaction.register_number = 0;

            setAckFlag();
            clearSIFlag();
            break;
        case slaveDataRecdAckReturned: //80
            if(sTransaction.next_is_register)
            {
                sTransaction.register_number = mpI2CRegs->I2DAT;
                sTransaction.next_is_register = false;
            }
            else
            {
                sTransaction.pSlaveData[sTransaction.register_number] = mpI2CRegs->I2DAT;
                sTransaction.register_number++;
                sTransaction.register_number %= sTransaction.bufferSize; //This depends on the size of the data array.
        ...
        case slaveDataTxedAckRecd: //B8

            mpI2CRegs->I2DAT = sTransaction.pSlaveData[sTransaction.register_number];
            sTransaction.register_number++;
            sTransaction.register_number %= sTransaction.bufferSize; //This depends on the size of the data array.
            setAckFlag();
            clearSIFlag();
        ...
        case slaveStopRecd: // A0
            setAckFlag();
            clearSIFlag();
            break;

        case slaveReadSelectedAckReturned: //A8
            mpI2CRegs->I2DAT = sTransaction.pSlaveData[sTransaction.register_number];
            sTransaction.register_number++;
            sTransaction.register_number %= sTransaction.bufferSize; //This depends on the size of the data array.
            setAckFlag();
        ...
        case arbitLostAckdAsWSlave: //no break

        case genCallRcdAcked:       //no break

        case arbitLostGenCallAcked: //no break

        case dataRcdNackRetnd:      //no break

        case genCallDataRcdAckRetnd://no break

        case genCallDataRcdNackRetnd://no break

        case arbitLostAckdAsRSlave: //no break

        case lastDataTxedAckRecd:   //no break
