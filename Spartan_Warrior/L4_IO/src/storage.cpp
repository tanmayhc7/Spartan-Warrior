...
FRESULT Storage::copy(const char* pExistingFile, const char* pNewFile, unsigned int* pReadTime, unsigned int* pWriteTime, unsigned int* pBytesTransferred)
{
    FRESULT status;
    FIL srcFile;
    FIL dstFile;
    ...
    for (;;) {
        ...
        if (FR_OK != (status = f_read(&srcFile, buffer, sizeof(buffer), &bytesRead)) || 0 == bytesRead) {
            break;
        }
        readTimeMs += sys_get_uptime_ms() - startTime;

        startTime = sys_get_uptime_ms();
        if (FR_OK != (status = f_write(&dstFile, buffer, bytesRead, &bytesWritten)) || bytesWritten != bytesRead) {
            break;
        }
 ...
 * */
unsigned long Storage::getfileinfo(const char* pFilename)
{
    FIL file;
    unsigned long size=0;
    FRESULT status = f_open(&file, pFilename, FA_OPEN_ALWAYS | FA_READ);
    if (FR_OK == status) {
        size  = f_size(&file);
        f_close(&file);
